import { ExpressiveCodeLine, type ExpressiveCodePlugin, InlineStyleAnnotation } from '@expressive-code/core'
...
import type { ThemedToken, ShikiTransformer } from 'shiki/core'
        ...
        hooks: {
            performSyntaxAnalysis: async ({ codeBlock, styleVariants, config: { logger } }) => {
                ...
                if (loadedLanguageName !== codeBlock.language) {
                    logger.warn(
                        `Found unknown code block language "${codeBlock.language}" in ${codeBlock.parentDocument?.sourceFilePath ? `document "${codeBlock.parentDocument?.sourceFilePath}"` : 'markdown/MDX document'
